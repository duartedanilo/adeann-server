...
def configuration():
    return {"NUMBER_OF_NINT": NUMBER_OF_NINT, "NENT": NENT,"NINT1": NINT1, "NINT2": NINT2, "LEARNING_RATE": LEARNING_RATE,
...
previsores = np.arange(total_by_num_steps * num_steps * NENT)
# np.ndarray(shape=(381-90,1,5))
preco_real = np.arange(total_by_num_steps * 1 * 1)
previsores = previsores.reshape(total_by_num_steps, num_steps, NENT)
...
lista_previsores = np.reshape(lista_previsores, (lista_previsores.shape[0], lista_previsores.shape[1], NENT))
...
if NUMBER_OF_NINT is 1:
    regressor.add(LSTM(units=NINT1, return_sequences=False, input_shape=(lista_previsores.shape[1], NENT)))
    regressor.add(Dense(units=1, activation=ACTIVATION_FUNCTION_N1))

if NUMBER_OF_NINT is 2:
    regressor.add(LSTM(units=NINT1, return_sequences=True, input_shape=(lista_previsores.shape[1], NENT),
...
lista_previsores_test = np.reshape(
    lista_previsores_test, (lista_previsores_test.shape[0], lista_previsores_test.shape[1], NENT))
...
try:
    cmd = ' '.join(["c_exec\\executavel.exe ", str(NENT), str(1), str(int(NUMBER_OF_NINT)+2), str(NINT1), str(NINT2)])
