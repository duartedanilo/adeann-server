...
def configuration():
    return {"NUMBER_OF_NINT": NUMBER_OF_NINT, "NINT1": NINT1, "NINT2": NINT2, "LEARNING_RATE": LEARNING_RATE,
...
def write_evaluation(pred, test):
    evaluation_list = ['{ "PREDICTION": ' + str(pred[i]) + ',  "REAL": ' + str(test[i]) + " },\n" for i in
                       range(len(test))]
...
def setConfigurarion(argv):
    global NUMBER_OF_NINT, NINT1, NINT2, ACTIVATION_FUNCTION1, ACTIVATION_FUNCTION2, LEARNING_RATE, OPTIMIZER, BATCH_SIZE, EPOCHS, NUMBER_OF_SUBJECT, SIMULATION_NAME, SIMULATION_PATH, EPOCHS_PRINT_CALLBACK
    if str(argv[1]) is "1":
        NUMBER_OF_NINT = 1
        NINT1 = int(getArgs(str(argv[2]), NINT1))
        ACTIVATION_FUNCTION1 = getArgs(str(argv[3]), ACTIVATION_FUNCTION1)
        LEARNING_RATE = float(getArgs(str(argv[4]), LEARNING_RATE))
        OPTIMIZER = getOptimizer(str(argv[5]), LEARNING_RATE)
        BATCH_SIZE = int(getArgs(str(argv[6]), BATCH_SIZE))
        EPOCHS = int(getArgs(str(argv[7]), EPOCHS))
        NUMBER_OF_SUBJECT = int(getArgs(str(argv[8]), NUMBER_OF_SUBJECT))
        SIMULATION_NAME = getArgs(str(argv[9]), SIMULATION_NAME)
        SIMULATION_PATH = getArgs(str(argv[10]), SIMULATION_PATH)
        EPOCHS_PRINT_CALLBACK = LambdaCallback(
        ...
        NUMBER_OF_NINT = 2
        NINT1 = int(getArgs(str(argv[2]), NINT1))
        NINT2 = int(getArgs(str(argv[3]), NINT2))
        ACTIVATION_FUNCTION1 = getArgs(str(argv[4]), ACTIVATION_FUNCTION1)
        ACTIVATION_FUNCTION2 = getArgs(str(argv[5]), ACTIVATION_FUNCTION2)
        ...
        NUMBER_OF_SUBJECT = int(getArgs(str(argv[10]), EPOCHS))
...
def argsFormatted():
    ...
    if numberOfHiddenLayers is 1:
        ...
        print("Learning Rate: " + str(argv[4]))
        print("Optimizer: " + str(argv[5]))
        print("Batch size: " + str(argv[6]))
        print("Epochs:     " + str(argv[7]))
        print("Simulation name:     " + str(argv[8]))
        print("Path:     " + str(argv[9]))
    ...
    if numberOfHiddenLayers is 2:
        ...
        print("Learning Rate: " + str(argv[6]))
        print("Optimizer: " + str(argv[7]))
        print("Batch size: " + str(argv[8]))
        print("Epochs:     " + str(argv[9]))
        print("Simulation name:     " + str(argv[10]))
        print("Path:     " + str(argv[11]))


x_train, y_train = load_dataset(pd.read_csv('dataset/train.csv', encoding='utf-8', delimiter=';', header=None))
x_test, y_test = load_dataset(pd.read_csv('dataset/test.csv', encoding='utf-8', delimiter=';', header=None))
...
lista_previsores = np.reshape(lista_previsores, (lista_previsores.shape[0], lista_previsores.shape[1], 5))
...
if NUMBER_OF_NINT is 1:
    regressor.add(LSTM(units=NINT1, return_sequences=False, input_shape=(lista_previsores.shape[1], 5)))
    regressor.add(Dense(units=1, activation='linear'))

if NUMBER_OF_NINT is 2:
    regressor.add(LSTM(units=NINT1, return_sequences=True, input_shape=(lista_previsores.shape[1], 5),
                       activation=ACTIVATION_FUNCTION1))
    regressor.add(LSTM(units=NINT2, return_sequences=False, activation=ACTIVATION_FUNCTION2))
    regressor.add(Dense(units=1, activation='linear'))
...
emitter(write_evaluation(pred_treino, lista_preco_real))
...
for i in range(len(lista_preco_real_test)):
    file.write("{ 'PREDICTION': " + str(pred_teste[i]) + ",  'REAL': " + str(lista_preco_real_test[i]) + " },\n")
